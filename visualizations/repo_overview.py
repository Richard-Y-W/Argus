...
# Figure 2: experiment verdict history
verdicts=['Supported','Supported','Partial','Rejected','Rejected','Rejected','Inconclusive','Rejected','Rejected'];palette={'Supported':'#15803d','Partial':'#ca8a04','Rejected':'#b91c1c','Inconclusive':'#64748b'}
fig,ax=plt.subplots(figsize=(8,2.4));y=np.arange(1,10);ax.scatter(y,[1]*9,s=260,c=[palette[v] for v in verdicts],edgecolor='white',linewidth=1.5)
for i,v in enumerate(verdicts,1):ax.text(i,1.0,str(i),ha='center',va='center',color='white',weight='bold',fontsize=8);ax.text(i,.91,v,ha='center',va='top',rotation=35,fontsize=8)
ax.set_xlim(.4,9.6);ax.set_ylim(.78,1.16);ax.set_yticks([]);ax.set_xticks([]);ax.spines['left'].set_visible(False);ax.spines['bottom'].set_visible(False);ax.set_title('Nine preregistered cycles: negative results are the majority');fig.tight_layout();fig.savefig(OUT/'experiment_verdicts.png',bbox_inches='tight');plt.close(fig)
